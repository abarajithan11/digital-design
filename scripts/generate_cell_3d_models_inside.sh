...
for arg in "$@"; do
  case "$arg" in
    ...
    --help|-h)
      cat <<'EOF'
Usage: generate_cell_3d_models_inside.sh [--overwrite] [pdk...]

Generate SCAD and GLB files from GDS cell libraries (container-only, no PNG rendering).
...
Environment variables:
  ...
  OVERWRITE        1 to regenerate scad/glb (default: 0)
EOF
      exit 0
      ;;
    --*)
...
for pdk in "${pdks[@]}"; do
  ...
  for cell in "${cells[@]}"; do
    ...
    if [ "$need_glb" = "0" ]; then
      printf 'skip %s %s: artifacts exist (%s)\n' "$pdk" "$cell" "$glb"
