...
The board's serial port is detected automatically. We send in 32-byte chunks
(the bridge's buffer size) and read each chunk back, since there is no hardware
flow control.
"""
import random
import serial
from utils_port import get_uart_port
...
got = bytearray()
with serial.Serial(PORT, BAUD, timeout=2, exclusive=True) as ser:
